...
export const dbService = {
  getTweet(twitterId: string): ProcessedTweet | null {
    const stmt = db.prepare('SELECT * FROM processed_tweets WHERE twitter_id = ?');
    const row = stmt.get(twitterId) as any;
    if (!row) return null;
    return {
  ...
  saveTweet(tweet: ProcessedTweet) {
    const stmt = db.prepare(`
      ...
      (twitter_id, twitter_username, bsky_uri, bsky_cid, bsky_root_uri, bsky_root_cid, status)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(
      tweet.twitter_id,
